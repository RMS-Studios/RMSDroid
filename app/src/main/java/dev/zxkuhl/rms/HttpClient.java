...
public class HttpClient {
    ...
    public static String rmsRuntime;
    public static String rmsMobileRuntime;

    public static void fetchrms(Activity activity) throws IOException {
        if (rmsRuntime != null) return;

        var res = activity.getResources();
        try (var is = res.openRawResource(R.raw.rms_mobile)) {
            rmsMobileRuntime = readAsText(is);
        }

        var conn = fetch(Constants.JS_BUNDLE_URL);
        try (var is = conn.getInputStream()) {
            rmsRuntime = readAsText(is);
        }
    }
